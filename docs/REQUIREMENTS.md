...
| --- | --- | --- |
| 8.1 | Enforce the contract version gate, including the minor (§4.3), and fail closed in platform tables it builds — on an unrecognized key, and on a value from an open vocabulary it does not implement (§4.4). | `component-class-absent`, `contract-major-mismatch`, `contract-malformed`, `contract-missing`, `contract-too-new`, `contract-under-declared`, `extension-kind-unimplemented`, `key-required`, `prerequisite-condition-unstated`, `toml-invalid`, `type-invalid`, `unknown-key`, `unknown-top-level` |
